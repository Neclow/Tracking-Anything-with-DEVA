...
class ResultSaver:
    def __init__(
        ...
        *,
        ...
        elif self.dataset == "burst" or self.dataset == "custom":
    ...
    while True:
        ...
        if saver.json_style == "vipseg":
            ...
            this_annotation = {
                "file_name": frame_name[:-4] + ".jpg",
                "segmentations": [
                    {"id": seg["id"], "score": seg["score"], "rle": seg["rle_mask"]}
                    for seg in segments_info
                ],
            }
            saver.all_annotations.append(this_annotation)
        elif saver.json_style == "custom":
            for seg in segments_info:
                seg["mask"] = mask == seg["id"]
                seg["area"] = int(seg["mask"].sum())
                coco_mask = mask_util.encode(np.asfortranarray(seg["mask"].numpy()))
                ...
                assert saver.object_manager.use_long_id
                assert prompts is not None

                out_mask = mask.numpy().astype(np.uint32)
                rgb_mask = np.zeros((*out_mask.shape[-2:], 3), dtype=np.uint8)
                for obj_id in all_obj_ids:
                    colored_mask = saver.id2rgb_converter._id_to_rgb(obj_id)
                    obj_mask = out_mask == obj_id
                ...
                "segmentations": [
                    {
                        ...
                        "bbox_ll": seg["ll"],
                        "bbox_lr": seg["lr"],
                        "bbox_diag": seg["bbox_diag"],
                        "centroid": seg["centroid"],
                        "area_bbox": seg["area_bbox"],
                        "area_obj_bbox_ratio": seg["area"] / seg["area_bbox"],
                        "width_height_ratio": seg["width_height_ratio"],
                    }
                    for seg in segments_info
                ],
            }
